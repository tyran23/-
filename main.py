...
#Переключатель, включающий и выключающий приписывание почтового индекса к полному адресу объекта.(Синяя кнопка)
#+ Автоматически при нажатии меняется адрес
...
class Button:
    ...
    def write_text(self, surface, text, text_color, length, height, x, y):
        ...
        surface.blit(my_text, (x, y + height / 3))
        return surface
...
def search(text):
    global coords, pt, address_text, post_text
    coords[0], coords[1] = get_coordinates(text)
    pt[0], pt[1] = get_coordinates(text)
    address_text = get_address(text)
    if post_text == "Off":
        address_text += get_post(text)
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if search_button.pressed(pygame.mouse.get_pos()):
                ...
                searching = False
            if reset_button.pressed(pygame.mouse.get_pos()):
                pt = [0, 0]
                address_text = " "
                show_map()
            if post_button.pressed(pygame.mouse.get_pos()):
                if post_text == "On":
                    post_text = "Off"
                else:
                    post_text = "On"
    ...
    try:  # Рисуем кнопки
        if searching:
            ...
            draw_buttons(search_button, 221, 221, 221, 0, 0, 600, 40, 0, "Поиск", 0, 0, 0)
        draw_buttons(reset_button, 221, 221, 221, 550, 400, 70, 70, 0, "Сброс", 0, 0, 0)
        draw_buttons(address_button, 221, 221, 221, 20, 410, 500, 40, 0, address_text, 0, 0, 0)
        draw_buttons(post_button, 38, 63, 140, 550, 350, 75, 50, 0, post_text, 0, 0, 0)
